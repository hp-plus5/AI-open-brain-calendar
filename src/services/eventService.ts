...
/** Number of rows fetched per Supabase request (the default server-side cap) */
const EVENTS_PER_PAGE = 1000

// ─── Queries ──────────────────────────────────────────────────────────────────

/**
 * Load all calendar events, paginating through results.
...
export async function loadEvents(): Promise<{ events: CalendarEventWithLocation[]; error: string | null }> {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .range(from, from + EVENTS_PER_PAGE - 1)

    if (error) return { events: [], error: error.message }
    if (!data || data.length === 0) break
    allEvents = allEvents.concat(data as CalendarEventWithLocation[])
    if (data.length < EVENTS_PER_PAGE) break  // last page
    from += EVENTS_PER_PAGE
  }
 ...
 * Returns null if the calendar has no events (for the export flow).
 */
export async function fetchEventsForCalendar(calendarId: string): Promise<{
  ...
  const eventIds = (links as { event_id: string }[]).map(link => link.event_id)
...
/** Create a new location row and return its generated id */
...
/** Create a new calendar event row and return the new row's id */
...
/** Delete a calendar event entirely (cascades to child exception rows via foreign key) */
...
/**
 * Insert a cancelled child row to suppress one occurrence of a recurring event.
 * The master event's rrule will skip dates that have a matching cancelled exception row.
 */
...
/**
 * Flip is_cancelled to true on an existing override row.
 * The row must NOT be deleted — that would remove the recurrence_id anchor and allow
 * the master event's rrule to regenerate the original occurrence.
 */
...
/**
 * Create an exception row representing a drag-and-dropped occurrence of a recurring
 * event at its new time. Returns the new row's id so calendar memberships can be
 * copied to it.
 */
...
export async function copyCalendarMemberships(
  ...
  const parentCalendarIds = calendarIdsMap.get(fromEventId) ?? []
  if (parentCalendarIds.length === 0) return
  await supabase.from('calendar_event_calendars').insert(
    parentCalendarIds.map(calendarId => ({
      event_id:    toEventId,
      calendar_id: calendarId,
      user_id:     userId,
    }))
